...
import * as ApiService from '../services/apiService'; // Đảm bảo đường dẫn đúng
...
const ChatScreen = () => {
    ...
    launchImageLibrary({ mediaType: 'photo' }, async (response) => {
      if (response.didCancel) {
        ...
        try {
          ...
          if (data.message) { // Trường hợp backend trả về message (độ chính xác thấp)
            botResponse = data.message;
          }
           addMessage({ id: `bot-${Date.now()}`, text: botResponse, user: 'bot' });

        } catch (error) {
          addMessage({ id: `bot-${Date.now()}`, text: `Lỗi: ${error.message}`, user: 'bot' });
        } finally {
          setIsLoading(false);
        }
      }
  ...
  const handleSendText = async () => {
    ...
    const userMessage = { id: `user-${Date.now()}`, text: inputText, user: 'user' };
    ...
    try {
      ...
      const botAnswer = data.answers[0]?.answer || "Tôi chưa có câu trả lời cho câu hỏi này.";
    ...
    return (
      <View style={[styles.messageRow, { justifyContent: isUser ? 'flex-end' : 'flex-start' }]}>
        <View style={[styles.messageBubble, isUser ? styles.userBubble : styles.botBubble]}>
          {item.text && <Text style={styles.messageText}>{item.text.replace(/\*\*/g, '')}</Text>}
