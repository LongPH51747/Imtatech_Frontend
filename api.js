...
export const apiSearchProducts = query =>
  axios.get(
    `${BASE_URL}/api/products/search-product?q=${encodeURIComponent(query)}`,
  );
...
// API để lấy thông báo - cần backend implement
export const apiGetNotifications = async userId => {
  try {
    const res = await axios.get(
      `${BASE_URL}/api/notifications/getByUserId/${userId}`,
    );
    return res.data;
  } catch (error) {
...
export const apiCreateAddress = async (userId, addressData) => {
  try {
    const res = await axios.post(
      `${BASE_URL}/api/address/create/userId/${userId}`,
      addressData,
    );
    return res.data;
  } catch (error) {
    throw new Error(error.response?.data?.message || 'Lỗi khi tạo địa chỉ');
  }
};

// API để cập nhật địa chỉ
export const apiUpdateAddress = async (addressId, addressData) => {
  try {
    const res = await axios.put(
      `${BASE_URL}/api/address/update/${addressId}`,
      addressData,
    );
    return res.data;
  } catch (error) {
...
export const apiDeleteAddress = async addressId => {
  try {
    const res = await axios.delete(
      `${BASE_URL}/api/address/deleteAddress/${addressId}`,
    );
    return res.data;
  } catch (error) {
